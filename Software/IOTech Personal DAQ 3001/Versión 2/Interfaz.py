...
class AcquisitionApp:
    def __init__(self, root):
        ...
        # Definir primero buffer_label y buffer_entry antes del OptionMenu
        ...
        self.buffer_entry.insert(0, "50000") # 1 
        self.buffer_entry.grid(row=4, column=1)

        # Línea combinada: cantidad + unidad
        ...
        # Canal config
        self.channel_list = [0]
        self.channel_label = ttk.Label(root, text="Canales: [0]")
    ...
    def add_channel(self):
        last_channel = self.channel_list[-1]
        if last_channel < 15:
            self.channel_list.append(last_channel + 1)
    ...
    def run_acquisition(self):
        ...
        gains = [gain] * num_channels
        flags_list = [flags] * num_channels
        ...
        if acq_mode == "finito" and duration_or_iters is not None:
            ...
            while self.running:
                ...
                if duration_or_iters is not None:
                    if mode == "tiempo":
                        if (time.time() - start_time) > duration_or_iters:
                            self.status_label.config(text="Adquisición detenida por duración.")
                            break
                    elif mode == "iteraciones":
                        if total_count >= duration_or_iters:
