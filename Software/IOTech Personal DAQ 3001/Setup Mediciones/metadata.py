...
def decode_flags(flag_value):
    ...
    for bitmask, name in FLAG_NAMES.items():
        if bitmask != 0 and (flag_value & bitmask) == bitmask:
            nombres.append(name)
    # Casos especiales de combinaciones conocidas
    if flag_value == 0:
        nombres.append("DafAnalog+Unipolar+SingleEnded")  # Caso más común por defecto
    return nombres
...
def actualizar_metadata_final(file_name, total_scans):
    meta_file = file_name.replace(".bin", "_metadata.json") # os.path.splitext(file_name)[0] + ".json"
