...
def get_amplitude_and_phase(ts_reference, reference, ts_signal, signal, cutoff_freq=10):
...
def lock_in(times, volts, cutoff_freq=10, detrend=False, plot_results=False): # Para DAQ con canal 1 referencia y canal 2 señal.
    t_ref, volts_ref = times[0], volts[0]  # referencia: canal 1
    t_signal, volts_signal = times[1], volts[1]  # señal: canal 2

    A_t, phi_t = get_amplitude_and_phase(t_ref, volts_ref, t_signal, volts_signal, cutoff_freq=1e2)
